...
class Session(object):
    ...
    def launch_name(self):
        if self.launch_key:
            name = "launch configuration \"" + self.launch_key + "\""
        else:
...
def load(env):
    ...
    if file_name:
        try:
            ...
            session = session or {}
            breakpoints = map(lambda b: Breakpoint(decode_path(b.get("file_name")), b.get("line")),
                              session.get("breakpoints", []))
            breakpoints = filter(lambda b: b.is_meaningful(), breakpoints)
            launches_list = map(
                lambda c: Launch(c.get("name"), c.get("main_class"), c.get("args"), c.get("remote_address")),
...
def save(env, data):
    ...
    if file_name:
        session = {}
        session["breakpoints"] = map(lambda b: {"file_name": encode_path(b.file_name), "line": b.line},
                                     data.breakpoints)
        session["launch_configs"] = map(
            lambda c: {"name": c.name, "main_class": c.main_class, "args": c.args, "remote_address": c.remote_address},
            data.launches.values())
